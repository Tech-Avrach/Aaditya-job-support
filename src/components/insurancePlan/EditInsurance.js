...
import { updateInsurance } from "../../redux/actions/insurance";
import { useNavigate, useParams } from "react-router-dom";
import insuranceService from "../../redux/services/insurance.service";
...
const EditInsurance = () => {
    ...
    useEffect(() => {
        insuranceService.get(id).then((res) => {
            console.log('retrive one insurence',res)
            setCurrentPrice(res?.data?.roleInfo?.price);
            setCurrentCurrency(res?.data?.roleInfo?.currency);
            setCurrentDuration(res?.data?.roleInfo?.duration);
        })
    },[])
        ...
        if (
          ...
          if (errorCount === 0) {
            const insuranceData = {
              ...
              dispatch(updateInsurance(id, insuranceData))
              .then((response) => {
                toast("Insurance Plan Updated successfully!", {
                  transition: Slide,
